...
class Solution():
    ...
    def getNeighbours(self, index0, curr_path):
        ...
        combined_path = curr_path + self.incident_list + ["*"]

        if up_index in combined_path:
            up_value = int(self.largeNum)
        if down_index in combined_path:
            down_value = int(self.largeNum)
        if left_index in combined_path:
            left_value = int(self.largeNum)
        if right_index in combined_path:
            right_value = int(self.largeNum)

        values = [up_value, down_value, left_value, right_value]

        if values == [largeNum, largeNum, largeNum, largeNum]:
            return 0, [0, 0], "Start Anew"
    ...
    def takenElement(self, index0):
        row, col = index0[0], index0[1]
        self.matrix[row, col] = self.largeNum
    ...
    def oneSnake(self, s):
        ...
        curr_path = [coords]
        for i in range(s-1):
            val, index, direction = self.getNeighbours(index0, curr_path)
            if direction == "start anew":
                return self.oneSnake(s)
            points.append(val)
        ...
        self.incident_list.append(curr_path)
        self.no_start_list.append(curr_path[0])
        return output
